...
class UserAdmin(ModelView, model=users):
    ...
    form_columns = [users.name, users.email, users.phone, users.address, users.email_verified, users.phone_verified]
    name = "User"
    name_plural = "Users"


class PostAdmin(ModelView, model=posts):
    column_list = [posts.id, posts.title, posts.content, posts.published, posts.owner_id, posts.created_at]
...
class CommentAdmin(ModelView, model=comments):
    column_list = [comments.comment_id, comments.comment, comments.post_id, comments.user_id, comments.commentree, comments.created_at]
    name = "Comment"
