...
def save_users(
    ...
    new_user: models.users = models.users(**users.model_dump(exclude="re_password"))
    user_exits = (
        db.query(models.users).filter(models.users.email == users.email).first()
    )
    if user_exits:
        raise HTTPException(
    ...
    user_exits = (
        db.query(models.users).filter(models.users.phone == users.phone).first()
    )
    ...
    if users.password == users.re_password:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=f"Password didn't Match"
        )
...
def update_user(
    ...
    if update:
        update = user_info.update(
            updated_user.model_dump(exclude=["re_password", "old_password"]),
