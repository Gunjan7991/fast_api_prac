...
@router.post(
    "/comments", status_code=status.HTTP_201_CREATED, response_model=schemas.comment
)
...
def add_comment(
    ...
    if not post:
        raise HTTPException(
            ...
            detail=f"No post with id: {comment.post_id}",
        )
    ...
    db.commit()
    logger.info(
        f"comment: {new_comment.comment_id, new_comment.comment, new_comment.user_id, new_comment.post_id, new_comment.commentree, new_comment.created_at}"
    )
...
def update_comment(
    ...
    if not my_comment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No comment with id: {comment_id} found!",
        )

    if my_comment.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"User can only modify thier comments!!",
        )
    ...
    logger.info(
        f"comment: {type(my_comment), my_comment.comment_id, my_comment.comment, my_comment.user_id, my_comment.post_id, my_comment.commentree, my_comment.created_at}"
    )
